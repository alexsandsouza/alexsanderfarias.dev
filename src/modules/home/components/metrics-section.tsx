    ...
    <section className="py-16 border-b border-border-subtle bg-surface/40">
      <Container size="wide">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-fg font-display">
              {dict.metrics.title}
            </h2>
          ...
          <div className="inline-flex items-center gap-1.5 text-xs font-mono text-fg-secondary bg-surface border border-border-subtle px-3 py-1.5 rounded-md">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-2xl sm:text-3xl font-extrabold font-mono text-fg group-hover:text-accent transition-colors">
                    {metric.value}
                  </span>
                  {metric.isPlaceholder && (
                    <span className="text-[10px] font-mono text-fg-secondary bg-surface border border-border-subtle px-1.5 py-0.5 rounded">
                      pendente
                    </span>
                  )}
                </div>
                <p className="text-xs text-fg-secondary mt-3 font-medium leading-snug">
                  {label}
                </p>
              </div>
            );
